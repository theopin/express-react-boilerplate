...
class Postgres implements Database {
  ...
  async createEntity (data: any): Promise<any> {
    return this.db.one(`INSERT INTO entities  (username, password, email) VALUES ('${data.username}', '${data.password}', '${data.email}') RETURNING *`)
  }

  async getOneEntity (_id: string): Promise<any> {
    return this.db.oneOrNone('SELECT * FROM entities WHERE _id = $1', [_id])
  }

  async getOneEntityByUsername (username: string): Promise<any> {
    return this.db.oneOrNone('SELECT * FROM entities WHERE username = $1', [username])
  }

  async getAllEntities (params: Partial<any>): Promise<any> {
    if (params === null || params === undefined) {
      return this.db.manyOrNone('SELECT * FROM entities')
    }

    const condition = Object.keys(params).map((key, index) => `$${index + 1}:${key}`).join(' AND ')
    const values = Object.values(params)
    return this.db.manyOrNone(`SELECT * FROM entities WHERE ${condition}`, values)
  }
  /* eslint-disable no-template-curly-in-string */

  async updateEntity (_id: string, newData: Partial<any>): Promise<any> {
    return this.db.oneOrNone('UPDATE entities SET ${this:name} WHERE _id = $1 RETURNING *', [_id, newData])
  }

  async deleteEntity (_id: string): Promise<any> {
    return this.db.oneOrNone('DELETE FROM entities WHERE _id = $1 RETURNING *', [_id])
  }
}
